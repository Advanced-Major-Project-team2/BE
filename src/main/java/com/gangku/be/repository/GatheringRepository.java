...
""")
    ...
    JOIN FETCH g.host
    WHERE g.status <> com.gangku.be.constant.gathering.GatheringStatus.FINISHED
      AND g.date BETWEEN :dateFrom AND :dateTo
""",
            countQuery =
                    """
    SELECT COUNT(g)
    ...
    WHERE g.status <> com.gangku.be.constant.gathering.GatheringStatus.FINISHED
      AND g.date BETWEEN :dateFrom AND :dateTo
""")
    Page<Gathering> findLatestGatheringsInDateRange(
            @Param("dateFrom") LocalDateTime dateFrom,
            @Param("dateTo") LocalDateTime dateTo,
            Pageable pageable);

    // 날짜 범위 필터 - 카테고리 있는 최신순
    @Query(
            value =
                    """
    SELECT g
...
""",
    ...
    WHERE g.category = :category
      ...
      AND g.date BETWEEN :dateFrom AND :dateTo
""")
    Page<Gathering> findLatestGatheringsByCategoryInDateRange(
            @Param("category") Category category,
            @Param("dateFrom") LocalDateTime dateFrom,
            @Param("dateTo") LocalDateTime dateTo,
            Pageable pageable);

    // 날짜 범위 필터 - 카테고리 없는 인기순
    @Query(
            value =
                    """
    SELECT g
...
""",
    ...
    WHERE g.status <> com.gangku.be.constant.gathering.GatheringStatus.FINISHED
      AND g.date BETWEEN :dateFrom AND :dateTo
""")
    Page<Gathering> findPopularGatheringsInDateRange(
            @Param("dateFrom") LocalDateTime dateFrom,
            @Param("dateTo") LocalDateTime dateTo,
            Pageable pageable);

    // 날짜 범위 필터 - 카테고리 있는 인기순
    @Query(
            value =
                    """
    SELECT g
...
""",
    ...
    WHERE g.category = :category
      ...
      AND g.date BETWEEN :dateFrom AND :dateTo
""")
    Page<Gathering> findPopularGatheringsByCategoryInDateRange(
            @Param("category") Category category,
            @Param("dateFrom") LocalDateTime dateFrom,
            @Param("dateTo") LocalDateTime dateTo,
            Pageable pageable);
